...
class RelatoriosController extends Controller
{
    public function extrato_conta()
    {
        ...
        if (!temPermissao('relatorios_extrato_conta.acesso')) {
        ...
        $contas = [];
        $res = $conn->query("SELECT id, banco, agencia, conta FROM contas_bancarias WHERE ativa = 1 ORDER BY banco ASC, agencia ASC");
        if ($res) {
        ...
        $this->view('relatorios/extrato_conta', [
            'contas' => $contas
        ]);
    }

    public function situacao_pedidos()
    {
        if (!temPermissao('relatorios_situacao_pedidos.acesso')) {
            $_SESSION['flash_error'] = 'Acesso negado.';
            header('Location: index.php?r=home');
            exit;
        }
        $this->view('relatorios/situacao_pedidos');
    }

    public function pedidos_situacao_pdf()
    {
        $conn = $this->db;
        ...
        // Adjust paths for Controller location (app/Controllers) -> root is ../../
        $rootPath = realpath(__DIR__ . '/../../');

        if ($cfg && !empty($cfg['arquivo_path'])) {
            $bgPath = realpath($rootPath . '/' . $cfg['arquivo_path']);
        }
        ...
        if (!$bgPath || !is_file($bgPath)) {
            $candidates = [
                ...
                $rootPath . '/uploads/papel_timbrado/papel_timbrado.png',
                $rootPath . '/uploads/papel_timbrado.jpg',
                $rootPath . '/uploads/papel_timbrado/papel_timbrado.jpg'
            ];
